...
│   ├── dates.ts         # Shared date formatting (relative dates, day·date, month/year) + daysSinceLastSession
│   ├── records.ts       # Per-set PR detection (weight, reps, volume) against history
│   ├── recovery.ts      # Muscle recovery status (per-group days since trained) + rest day activity suggestions + group skip history + smart day suggestions
...
├── pages/
│   ├── Home.tsx            # Monthly calendar (color-coded by day type, smart suggestions for near-future days), bento stats grid, muscle recovery card, rest day suggestions, resume banner, backup/export
...
└── components/
    ...
    ├── Schedule.tsx             # Weekly program schedule view (Mon–Sun, exercises, recovery pills, staleness, smart warnings, start buttons)
...
- **Smart day suggestions** — `lib/recovery.ts` exports `getLiftDayGroups(day)` (maps a ProgramDay's exercises to muscle group labels) and `getSmartDaySuggestion(dow, dateOffset, programDays, recoveryStatuses)` which returns a `SmartSuggestion { type, reason?, suggestion? }`. For near-future days (today + 2 days), if a lift day's target muscles are recovering, it suggests an alternative lift day whose muscles are recovered, or cardio if none available. Beyond 2 days, falls back to the static program template. Used by Home calendar (amber rings for adapted days) and Schedule component (warning + suggestion text on lift cards).
...
- **Bento stats grid** — Home page displays stat cards: Streak (consecutive days), Total Workouts (1-col each), Last Session (col-span-2, day focus + relative date), This Month (col-span-2, mini calendar grid with M–S headers, color-coded circles by day type, smart rings for near-future days via `getSmartDaySuggestion` with amber rings for adapted days, static program rings for 3+ days out, legend with Lift/Cardio/Recovery/Planned/Adapted indicators, session count), Muscle Recovery (col-span-2, per-group recovery pills), and Rest Day Suggestions (col-span-2, context-aware activity nudges).
...
- **Schedule view** — `Schedule` component (`src/components/Schedule.tsx`) in the Progress page's "Schedule" tab. Shows the full weekly program (Mon–Sun) with day name, type badge (Lift/Cardio/Recovery/Rest), focus description, exercise list with rep ranges (lift days), descriptions + duration (cardio/recovery), tips (rest days), and "Start Workout" buttons. Today's card highlighted with a red ring. Recovery-enriched: shows per-muscle-group recovery pills (green=recovered, orange=recovering with day count) on all cards — lift days show targeted groups, other days show all tracked groups. Staleness line ("Last done Xd ago" / "Never done") on all cards. Lift days with recovering muscles show amber warning + alternative suggestion (e.g., "Chest still recovering — Consider Pull instead"). Legend above cards explains recovery pill colors. Uses `getEffectiveExercise()`, `getMuscleRecoveryStatus()`, `getLiftDayGroups()`, `getSmartDaySuggestion()`, and `daysSinceLastSession()`.
- **Shared date formatting** — `lib/dates.ts` provides `formatRelativeDate` ("Today"/"3 days ago"), `formatRelativeDateShort` ("3d ago"), `formatDayDate` ("Mon · Mar 14"), `formatMonthYear` ("March 2026"), and `daysSinceLastSession(dayId, history)` (days since a specific program day was last done, or null if never). Used by Home, History, HistoryEdit, and Schedule — avoid duplicating date logic in page components.
...
| `/progress` | Progress | Charts/Schedule toggle, muscle-group-categorized exercise picker, per-exercise charts (1RM, volume), color-coded PR dashboard, weekly program schedule |
