...
- **Home layout** — Quick Start section (resume banner, rolling-cycle hero card with recovery suggestion banner, quick cardio/rest log buttons), Stats section (Streak, Total Workouts, Last Session cards), interactive calendar (tappable cells — tap any date to log cardio/rest, undo non-lift logs, or move lift workouts via date picker; future rings use rolling cycle projection skipping logged dates), Muscle Recovery section, Rest Day suggestions, Backup & Export. Hero card adapts to all day types (lift/cardio/recovery/rest) with type-specific labels, metrics, and action buttons. If today already has a logged session, the hero advances to the next open cycle day and shows "Coming Up" with a date chip and an info card instead of a start button.
