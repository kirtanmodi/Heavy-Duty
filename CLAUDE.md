...
- **Gym equipment curation** — `lib/curatedWorkout.ts` defines a `GymEquipmentProfile` (30 equipment IDs mapped to boolean availability) and `curateWorkoutForFocus(focus, profile)` which selects the best exercise per slot from ranked candidates based on available equipment. Workout page shows a "Curate from My Gym" section on lift days (Push/Pull/Legs & Abs) where users toggle their gym's equipment, then rebuild the workout. Curation is locked once any set is logged to prevent overwriting data. Profile persists in `settingsStore.gymEquipment`. Store actions: `replaceActiveWorkoutExercises` (replaces all exercises + clears split supersets).
