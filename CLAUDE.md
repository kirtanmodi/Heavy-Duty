...
- **Muscle recovery status** — `lib/recovery.ts` exports `getMuscleRecoveryStatus(history)` which scans the last 30 lift entries and returns per-group recovery status (recovering <4 days, recovered 4d+, never trained). Displayed as a bento card on Home with color-coded pills (orange=recovering, green=ready). Workout page shows a recovery warning banner when targeted muscle groups are still recovering, with actionable "Skip [Group] this week" buttons that skip all exercises targeting a recovering muscle group in one tap.
- **Anti-chronic-skip safeguard** — `lib/recovery.ts` exports `getGroupSkipHistory(group, history)` which counts consecutive sessions where all exercises for a muscle group were skipped. Prevents muscle groups from being neglected: 0 consecutive skips = normal skip button, 1 consecutive skip = amber warning (still allows skip), 2+ consecutive skips = blocks skipping with red warning ("Train it today for balanced progress"). Ensures the Push/Pull/Legs rotation stays balanced even when overlapping groups (e.g. Arms = triceps on Push + biceps on Pull) would otherwise get chronically skipped.
