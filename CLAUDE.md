...
│   ├── History.tsx         # Past workouts with inline edit, delete, exercise filter (imports stats from lib/stats)
...
- **Cardio/rest day tracking** — program days with `type: 'cardio'` or `type: 'recovery'` show activity suggestions from `cardioActivities` map in `data/programs.ts` (includes Sunday rest activities). Users tap to select an activity before marking done. Workout page renders a "Mark as Done" button to log a cardio/recovery session via `logCardioSession(dayId, dayName, programName, dayType, activityName?)`. `WorkoutEntry.dayType` field (`DayType = 'lift' | 'cardio' | 'recovery' | 'rest'`) persists the session type. `WorkoutEntry.activityName` stores the selected activity. Calendar on Home is color-coded: green for lifts, blue for cardio, blue/60 for recovery, subtle dots for scheduled rest days with no activity. History cards show `activityName` for cardio/recovery entries.
- **Muscle recovery status** — `lib/recovery.ts` exports `getMuscleRecoveryStatus(history)` which scans the last 30 lift entries and returns per-group recovery status (recovering <4 days, recovered 4d+, never trained). Displayed as a bento card on Home with color-coded pills (orange=recovering, green=ready). Workout page shows a recovery warning banner when targeted muscle groups are still recovering.
