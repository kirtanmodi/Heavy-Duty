...
- **Workout date mutation** — `updateWorkoutDate(workoutId, dateKey)` store action allows moving a workout to a different date (re-sorts history after). Validated on the caller side: no future dates, no collisions.
