...
└── components/
    ...
    ├── Schedule.tsx             # Weekly program schedule view (Mon–Sun, exercises per day, start buttons)
...
- **Bento stats grid** — Home page displays stat cards: Streak (consecutive days), Total Workouts (1-col each), Last Session (col-span-2, day focus + relative date), This Month (col-span-2, mini calendar grid with M–S headers, color-coded circles by day type, colored rings for planned future days by program schedule, ring for today, legend, session count), Muscle Recovery (col-span-2, per-group recovery pills), and Rest Day Suggestions (col-span-2, context-aware activity nudges).
...
- **Schedule view** — `Schedule` component (`src/components/Schedule.tsx`) in the Progress page's "Schedule" tab. Shows the full weekly program (Mon–Sun) with day name, type badge (Lift/Cardio/Recovery/Rest), focus description, exercise list with rep ranges (lift days), descriptions + duration (cardio/recovery), tips (rest days), and "Start Workout" buttons. Today's card highlighted with a red ring. Uses `getEffectiveExercise()` for exercise name resolution.
