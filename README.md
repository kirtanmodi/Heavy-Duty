...
- **Mentzer warm-up/working set protocol** — each exercise gets 2 sets: a warm-up at 50% weight and a working set to failure, both at 4-1-4 tempo. Set labels ("W-up" / "Work") shown inline. Workout includes a manual "Back Off Today" button that reduces current non-skipped weighted sets by one exercise-specific increment without changing overload rules or history
...
- **Workout session UI** — session summary card (exercise count + set progress), Back Off Today, collapsible setup tools, toggleable coaching hints (overload suggestions), collapsible recovery actions
