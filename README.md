...
- **Mentzer warm-up/working set protocol** — each exercise gets 2 sets: a warm-up at 50% weight and a working set to failure, both at 4-1-4 tempo. Set labels ("W-up" / "Work") shown inline. Workout includes manual Back Off controls for the full session or one exercise; both reduce current non-skipped weighted sets by one exercise-specific increment without changing overload rules or history
- **Progressive overload** — automatic suggestions to increase weight, add reps, maintain, or reduce weight based on working-set (to-failure) performance
...
- **Progress charts & PR dashboard** — Charts/Schedule toggle. The exercise picker stays collapsed by default, opens into muscle-group tabs with full-width exercise rows, and shows horizontal PR chips on small screens. Schedule tab uses compact cards, a Legend toggle, expanded "Why this day is here" details, recovery pills, smart warnings, and start buttons. Already-logged days appear inline with a "Logged" chip and activity summary
...
- **Workout session UI** — session summary card (exercise count + set progress), Back Off Today, per-exercise Back Off from the exercise menu, collapsible setup tools, toggleable coaching hints (overload suggestions), collapsible recovery actions
...
- **PWA install** — Home-only native install prompt (Chrome/Edge) with iOS fallback instructions, app shortcuts (Open Workout, Progress, History), offline-ready via service worker
- **Rest timer** — configurable countdown with presets, optional auto-start on set completion, and optional completion sound alert
...
| `settingsStore` | `hd_settings` | Program selection, rest timer, auto-start timer, rest-timer sound alert, gym equipment profile, custom gym equipment. Action: `restoreState` (merges gym equipment over defaults) |
