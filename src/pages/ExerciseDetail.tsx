...
function ExerciseDetailContent({ exercise }: { exercise: Exercise }) {
  ...
  return (
    <PageLayout className="flex flex-col gap-8">
      ...
      {pairedExercise && (
        <section className="flex flex-col gap-2 rounded-2xl border border-accent-yellow/25 bg-accent-yellow/5 p-6">
      ...
      <section className="rounded-2xl border border-border-card bg-bg-card p-6">
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2 rounded-xl bg-bg-input p-4">
              <p className="text-xs text-text-muted">Tempo</p>
              <p className="text-xl font-semibold text-text-primary">4-1-4</p>
            </div>
            <div className="flex flex-col gap-2 rounded-xl bg-bg-input p-4">
            ...
            <div className="flex flex-col gap-2 rounded-xl bg-bg-input p-4">
              ...
              <p className="text-xl font-semibold text-text-primary">{exercise.restSeconds === 0 ? "Superset" : `${exercise.restSeconds}s`}</p>
            </div>
          </div>
      ...
      <section className={`flex flex-col gap-2 rounded-2xl border p-6 ${overloadColors[suggestion.type]}`}>
        <p className="text-xs font-semibold tracking-wide">Progressive Overload</p>
        <p className="text-base leading-relaxed">{suggestion.message}</p>
      </section>

      <section>
        <div className="mb-4 flex items-baseline justify-between">
          <h2 className="text-base font-medium text-text-secondary">Recent Sessions</h2>
        </div>
